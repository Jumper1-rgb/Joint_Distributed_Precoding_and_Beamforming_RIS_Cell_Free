function F = optFDistributed(L, K, Nt, H_lk, u_k, F, APpwr)
f_lk = F.f_lk;
F.f_lk = zeros(Nt, L, K);
% 生成辅助矩阵B
...
for l = 1:L
    ...
    while (abs(errVal)>errth) && (upper >= errth)
        ...
        for k = 1:K
            ...
            for ell = 1:L
                if ell ~= l
                    sumTemp = sumTemp + Lambda((l-1)*Nt+1:l*Nt,(ell-1)*Nt+1:ell*Nt)*f_lk(:,ell,k);
                end
            end
            F.f_lk(:,l,k) = (Lambda((l-1)*Nt+1:l*Nt,(l-1)*Nt+1:l*Nt)+lambda*eye(Nt))^(-1)*(B.b_lk(:,l,k) - sumTemp);
        end
